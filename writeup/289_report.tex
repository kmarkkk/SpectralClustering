...
\maketitle
\begin{abstract}
Clustering is one of the most popular techniques adopted in industry and research area to detect group structure in the data set.  K-Means and Spectral Clustering are two typical algorithms for grouping similar subset of data in a large data set. K-Means is simple and fast, but depends heavily on the initialization and is likely to stuck on the local optimal points. Spectral clustering often outperforms K-Means result by using eigenvectors of the similarity matrix. However, it’s computationally expensive and the simple implementation has a serious memory bottleneck. In order to gain some insights into the performance of these clustering methods, in this report we experienced with K-Means and Spectral clustering on Musk(xxx) dataset and compared clustering results. Moreover, we tried different optimizations and measured  how much they improve the clustering performance. For K-Means, we tried smart initialization techniques such as KM-2 and BF. For Spectral Clustering, we applied subsampling at the beginning to group data points by a distortion minimizing transformation, and conducted Spectral Clustering on the preprocessed data set. This optimization significantly reduce the time and memory needed by the algorithm while retaining comparable clustering accuracy.
\end{abstract}
